...
class TestExitAccountChrome:
    def test_success_click_constructor_chrome(self, login_in_account_chrome):
        ...
        text_login_account = WebDriverWait(login_in_account_chrome, 3).until(expected_conditions.visibility_of_element_located(Locators.BUT_LOGIN)).text
        # assert
        assert text_login_account == 'Войти'
...
class TestExitAccountFirefox:
    def test_success_click_constructor_chrome(self, login_in_account_firefox):
        ...
        text_login_account = WebDriverWait(login_in_account_firefox, 3).until(expected_conditions.visibility_of_element_located(Locators.BUT_LOGIN)).text
        # assert
        assert text_login_account == 'Войти'